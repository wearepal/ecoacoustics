...
def run_random_forest(representations, labels, n_folds=5):
    ...
    for i in tqdm(range(n_folds), desc="Random Forest Run"):
...
def apply_umap(representations, labels, label_names):
    ...
    mapper = umap.UMAP(metric='euclidean', min_dist=0, n_neighbors=50, random_state=0, n_components=2).fit(
        representations)
    labels_to_plot = []
    for lab in labels:
        labels_to_plot.append(label_names[lab])
